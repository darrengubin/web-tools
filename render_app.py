...
import uvicorn
import nest_asyncio
nest_asyncio.apply()  # 防止 TaskGroup 错误
from starlette.applications import Starlette
from starlette.routing import Mount, Route
from starlette.responses import JSONResponse

# 创建 ASGI app（兼容不同 mcp 版本）
# mcp >= 1.14.0 用 streamable_http_app()，否则用 sse_app()
if hasattr(mcp, 'streamable_http_app'):
    http_app = mcp.streamable_http_app()
    transport_type = "streamable-http"
elif hasattr(mcp, 'sse_app'):
    from mcp.server.fastmcp import settings
    mcp.settings.mount_path = "/mcp" if hasattr(mcp.settings, 'mount_path') else None
    http_app = mcp.sse_app("/mcp")
    transport_type = "sse"
else:
...
if __name__ == "__main__":
    ...
    print(f"  EAI config:      {json.dumps({'url': 'https://host/mcp', 'transport': 'streamable-http'})}")
