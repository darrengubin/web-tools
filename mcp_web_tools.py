...
def fallback_search(keyword: str, limit: int, timeout: int = 15) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # 第三备选：baidusearch 库（国内网络友好，无需 Key）
    if not results:
        try:
            from baidusearch.baidusearch import search as baidu_search
            baidu_items = baidu_search(keyword, num_results=limit)
            for item in baidu_items:
                ...
                if title or snippet:
                    results.append({
                        ...
                        "url": url,
                        "source": "baidusearch",
                        **date_evidence(title + " " + (snippet or "")),
                    })
            attempts.append({"source": "baidusearch", "ok": True, "count": len(results)})
        except Exception as exc:
            attempts.append({"source": "baidusearch", "ok": False, "error_type": classify_error(str(exc)), "message": str(exc)})
