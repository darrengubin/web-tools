...
def extract_jiemian_article_links(html: str) -> List[str]:
    """Extract article URLs from 界面新闻 search/list HTML."""
    urls: List[str] = []
    soup = BeautifulSoup(html or "", "lxml")
    for a in soup.find_all("a"):
        href = a.get("href", "")
        ...
        if re.search(r"jiemian\.com/article/\d+\.html", href) and ("汽车早报" in text or True):
            if href not in urls:
                urls.append(href)
    for m in re.finditer(r"https?://www\.jiemian\.com/article/\d+\.html|//www\.jiemian\.com/article/\d+\.html|/article/\d+\.html", html or ""):
        href = m.group(0)
        if href.startswith("//"):
            href = "https:" + href
        elif href.startswith("/"):
...
def discover_jiemian_auto_morning_links(start_date: str, end_date: str, timeout: int = 20, retries: int = 1, max_candidates: int = 30) -> dict:
    """Use 界面旧版搜索页 to find 汽车早报 articles, then verify by detail publish date."""
    search_url = "https://a.jiemian.com/index.php?m=search&a=index&opt=old&msg=%E6%B1%BD%E8%BD%A6%E6%97%A9%E6%8A%A5"
    last_error = None
    html = ""
    final_url = search_url
    for attempt in range(retries + 1):
        try:
            resp = requests_get(search_url, timeout, headers=DEFAULT_HEADERS)
            resp.raise_for_status()
            if not resp.encoding or resp.encoding.lower() == "iso-8859-1":
                resp.encoding = resp.apparent_encoding
            html = resp.text
            ...
            if attempt < retries:
                retry_sleep(attempt)
    if not html:
        return {"ok": False, "url": search_url, "message": f"Error: {last_error}", "error_type": classify_error(last_error or "")}

    candidate_urls = extract_jiemian_article_links(html)[:max_candidates]
    links_by_date: Dict[str, Dict[str, Any]] = {}
    ...
    return {"ok": True, "url": final_url, "candidate_count": len(candidate_urls), "checked": checked[:max_candidates], "links_by_date": links_by_date}
...
def collect_jiemian_auto_morning(start_date: str, end_date: Optional[str] = None, timeout: int = 20, retries: int = 1, max_candidates: int = 30) -> dict:
    """采集界面新闻汽车早报。通过旧版搜索页发现对应日期文章，再抓取详情页并拆分条目。"""
    end_date = end_date or start_date
...
@mcp.tool()
def collect_sina_auto_7x24(start_date: str, end_date: Optional[str] = None, pages: int = 5, limit: int = 20, timeout: int = 20, retries: int = 1) -> dict:
    """采集新浪汽车7x24快讯。来源：https://auto.sina.com.cn/7x24/?tagid=1，直接请求其 getTagnews 接口并按 cTime 过滤。"""
    end_date = end_date or start_date
    start = datetime.date.fromisoformat(start_date)
    end = datetime.date.fromisoformat(end_date)
...
def collect_autohome_newbrand(start_date: str, end_date: Optional[str] = None, timeout: int = 20, retries: int = 1, link_limit: int = 80, fetch_details: bool = True) -> dict:
    ...
    for cand in candidates:
        ...
        if date_in_range(event_date, start, end):
            items.append(build_new_car_item(title, title, url, "汽车之家上市新车", "autohome_newbrand", event_date, "high", "high"))
            continue
        if fetch_details:
            detail_checked += 1
            detail = fetch_article_content(url, timeout=min(timeout, 15), retries=0)
            detail_text = f"{detail.get('title', '')} {detail.get('content', '')}"
            detail_date = explicit_range_date(detail_text, start, end) or detail.get("date") or parse_date_from_text(detail_text, default_year=start.year)
            if date_in_range(detail_date, start, end):
                summary = detail.get("content") or title
                items.append(build_new_car_item(title, summary, url, "汽车之家上市新车", "autohome_newbrand", detail_date, "high", "medium"))
    items = filter_items_by_date(dedupe_items(items), start_date, end_date, strict=True)
    ...
    for source in sources:
        ...
        try:
            if source_type == "cls_auto_morning":
                ...
                result = collect_sina_auto_7x24(start_date=start_date, end_date=end_date, pages=int(source.get("pages", 5)), limit=int(source.get("limit", 20)), timeout=int(source.get("timeout", 20)), retries=int(source.get("retries", 1)))
                if result.get("ok"):
                    items.extend(result.get("items", []))
...
def collect_auto_5_sources(date: str, weibo_cookie: Optional[str] = None, resolve_original_links: bool = False) -> dict:
    ...
    sources = [
        ...
        {"name": "新浪汽车7x24快讯", "type": "sina_auto_7x24", "pages": 5, "limit": 20, "timeout": 20, "retries": 1},
