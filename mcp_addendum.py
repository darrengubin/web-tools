...
GOV_WEBSITE_DOMAINS = {
    ...
    "国务院":           "gov.cn",
    # 省/市政府
    "北京市":  "beijing.gov.cn",       "上海":  "shanghai.gov.cn",
    "天津市":  "tj.gov.cn",           "重庆":  "cq.gov.cn",
    "广东省":  "gd.gov.cn",           "深圳":  "sz.gov.cn",
    ...
    "山东":    "shandong.gov.cn",     "湖南":  "hunan.gov.cn",
...
def collect_all_sources(
    ...
    if resolve_links:
        ...
        for item in all_items:
            ...
            # 跳过已有好链接的
            if current_url and not is_aggregator(current_url):
                if any(d in current_url for d in [".gov.cn", "autohome.com.cn", "yiche.com", "dongchedi.com"]):
            ...
            # 策略2: 界面新闻子条目 → 搜索独立链接
                ...
                clean_t = title[:40]
                sr = web_search(f"{clean_t} 界面新闻", limit=5, retries=1)
                if sr.get("ok"):
                    for r in sr.get("results", []):
                        url = r.get("url", "")
                        if "jiemian.com/article" in url and url != current_url:
                            best_url = url
            ...
            # 策略3: 财联社子条目 → 搜索独立链接
                ...
                clean_t = title[:40]
                sr = web_search(f"{clean_t} 财联社", limit=5, retries=1)
                if sr.get("ok"):
                    for r in sr.get("results", []):
                        url = r.get("url", "")
                        if "cls.cn" in url or "clsi.com.cn" in url:
                            best_url = url
                            best_source = "财联社独立文章"
                            break
                if not best_source:
        ...
        # 统计链接质量
        good_links = sum(1 for i in all_items if i.get("url") and not is_aggregator(i["url"]) and "baidu.com/s" not in i["url"])
        total = len(all_items)
        link_quality = {
