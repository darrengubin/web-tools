...
def collect_all_sources(
    ...
    if resolve_links:
        # 对 autoinfo 的政府新闻补链
        gov_result = resolve_government_news_links(all_items, search_if_needed=True)

        # 对已有 url 做链接优先级评分
        link_result = resolve_news_original_links(
            items=all_items,
            default_date=date,
            limit=6,
        )
        link_quality = {
            "missing_original_link_count": link_result.get("missing_original_link_count", 0),
            "original_link_count": link_result.get("original_link_count", 0),
        }
        all_items = link_result.get("items", all_items)
